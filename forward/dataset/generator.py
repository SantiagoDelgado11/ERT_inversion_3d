...
def generate_single_sample(seed=None, mesh=None, config_geology=None, config_survey=None, return_mesh=False):
    ...
    electrodes = generate_surface_electrode_grid(
        num_x=config_survey.get('num_electrodes_x', 21),
        num_y=config_survey.get('num_lines', 5),
        spacing_x=config_survey.get('electrode_spacing_x', 2.0),
        spacing_y=config_survey.get('electrode_spacing_y', 4.0),
        center_x=0.0,
        center_y=0.0
    )
    ...
    if config_survey['arrays'].get('dipole_dipole', {}).get('active', False):
        ...
        a_max = config_survey['arrays']['dipole_dipole'].get('a_max', 6)
        sequence.extend(generate_dipole_dipole(
            num_electrodes_x=config_survey.get('num_electrodes_x', 21),
            num_lines=config_survey.get('num_lines', 5),
            max_n=max_n,
            a_max=a_max
        ))
